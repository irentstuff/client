...
export const ItemEditModal = ({ modalDetails, updateModalDetails, setFetchDataAgain }) => {
  ...
  const onFinish = () => {
    ...
    if (imageFolderUrl !== undefined) {
      ...
      uploadedImageUrl = uploadedFileList.map((img) => {
        let imageUrl = ''

        if (Object.hasOwn(img, 'originFileObj')) {
          imageUrl = `${imageFolderUrl}/${img.originFileObj.uid}_${moment().format('YYYYMMDD_HH:mm:ss')}`
      ...
      imagesToDelete.map((img) => deleteItemImageLocal(img))
    }
  ...
  useEffect(() => {
    const fileList = itemImagePath.map((item) => {
      return {
        url: item
      }
    })
